...
export const useBuilder = () => {
  ...
  const loadResume = useCallback(async () => {
    ...
    // 1. Check if there's an unsaved emergency local backup from a power cut/crash
    ...
    if (emergencyBackup) {
      ...
      setIsDirty(true); // Keep it dirty so it attempts to sync to DB on next pause
      setLoading(false);
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setIsDirty(false);
      // ✅ SUCCESS: Clear emergency backup once database safely handles it
  ...
  // 🛡️ ACCIDENTAL EXIT GUARD: Blocks unexpected page refreshes/closes if changes are unsaved
  useEffect(() => {
  ...
  // 💾 LOCAL MIRRORING: Save to localStorage instantly on keypress (Guards against power cuts)
  useEffect(() => {
    if (!resume || !id || !isDirty || PUBLIC_TEMPLATES.includes(id)) return;
    localStorage.setItem(`emergency_backup_${id}`, JSON.stringify(resume));
  }, [resume, id, isDirty]);

  // ⏱️ ANTI-WATERFALL AUTO SAVE LOOP: Extracts the handleSave trigger cleanly using a ref
  ...
  useEffect(() => {
    ...
    // Wait a solid 2000ms after user finishes typing before doing network call
    const timer = setTimeout(() => {
      saveRef.current();
    }, 2000);

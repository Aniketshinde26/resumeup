...
export default function TechTemplate({ data }: { data: ResumeData }) {
  return (
    <div
      id="resume-template"
      className="mx-auto w-[210mm] min-h-[297mm] bg-white p-[10mm] font-sans overflow-hidden box-border relative shadow-2xl print:shadow-none"
    >
      ...
      <div className="grid grid-cols-12 gap-8">
                      ...
                      {proj.link && (
                        <span className="text-[9px] font-mono">
                          {proj.link.startsWith('http') ? (
                            <a href={proj.link} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">
                              view_source
                            </a>
                          ) : (
                            <span className="text-slate-400 italic font-sans">{proj.link}</span>
                          )}
                        </span>
                      )}
                    </div>
        ...
        <div className="col-span-4 space-y-6">
          <section className="no-break">
            <h2 className="text-slate-900 font-black text-[11px] uppercase tracking-[0.2em] mb-4">
